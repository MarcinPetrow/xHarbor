...
const server = http.createServer(async (request, response) => {
  try {
    ...
    if (url.pathname === "/api/session" || url.pathname === "/api/session/presence" || url.pathname === "/api/users" || url.pathname === "/api/presence") {
...
server.listen(port, "127.0.0.1", () => {
  console.log(`xtalk-web listening on http://127.0.0.1:${port}`);
});
